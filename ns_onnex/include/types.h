//
// Created by Abdulbary on 2026/07/22.
//

#ifndef NS_LITE_AI_TYPES
#define NS_LITE_AI_TYPES
...
template<typename T1, typename T2>
typename lite::types::BoundingBoxType<T1, T2>::value_type
lite::types::BoundingBoxType<T1, T2>::area() const
{
  //types::__assert_type<value_type, score_type>();
    auto result = width() * height();
    return std::abs(result);// std::abs<value_type>(width() * height());
}
